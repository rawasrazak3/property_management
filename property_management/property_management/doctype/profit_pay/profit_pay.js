            ...
            frappe.call({
                ...
                callback: function(response) {
                    ...
                    unique_assets.forEach(asset => {
                        frappe.call({
                            method: 'property_management.property_management.doctype.profit_pay.profit_pay.fetch_assets',
                            args: {
                                property_id: asset.property_name,
                                limit_page_length: 100 // Adjust the limit as needed
                            },
                            callback: function(r) {
                                // Clear existing rows in the child table
...
function create_journal_entry(frm, mode_of_payment) {
    frappe.call({
        ...
        callback: function(response) {
            if (response.message) {
                frappe.db.set_value(frm.doctype, frm.doc.name, 'journal_entry', response.message)
                .then(() => {
                    // frappe.show_alert({ message: __('Journal Entry updated successfully'), indicator: 'green' });
                    frm.reload_doc(); // Reload the form to reflect the change
                });                
