...
def create_journal_entry_from_sales_invoice(sales_invoice):
    ...
    # Prevent duplicate JE for same Sales Invoice
    existing_je = frappe.get_all(
        "Journal Entry",
        filters={
            "custom_sales_invoice_": sales_invoice,
    ...
    if existing_je:
        frappe.logger().debug(f"JE already exists for {sales_invoice}, skipping.")
        return
    ...
    credit_amount = gl_entry[0].get("credit", 0)
    debit_amount = gl_entry[0].get("debit", 0)
    amount = round(credit_amount - debit_amount, 3)
    # Fetch property name from item table
    property_name = next(
        (item.item_name for item in doc.items if item.item_name), None
    )
    ...
    if amount > 0:
        ...
        # Credit shareholder accounts
        total_allocated = 0
        last_index = len(shareholders) - 1

        for i, shareholder in enumerate(shareholders):
            if i == last_index:
                share_amount = round(amount - total_allocated, 3)
            else:
                share_amount = round((amount * shareholder.contribution) / 100, 3)
                total_allocated += share_amount
...
def on_cancel_sales_invoice(doc, method):
    frappe.logger().debug(f"Sales Invoice {doc.name} cancelled. Cancelling linked documents.")
    ...
    journal_entries = frappe.get_all(
        "Journal Entry",
        filters={"custom_sales_invoice_": doc.name, "docstatus": 1},
        fields=["name"]
    )
    ...
    for je in journal_entries:
        try:
            ...
            frappe.logger().debug(f"JE {je.name} cancelled.")
        except Exception:
            frappe.log_error(frappe.get_traceback(), f"Error cancelling JE {je.name}")
            frappe.throw(f"Failed to cancel Journal Entry {je.name}")